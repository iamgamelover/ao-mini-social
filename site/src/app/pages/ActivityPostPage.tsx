...
import { AO_STORY, MINI_SOCIAL, TIP_IMG } from '../util/consts';
...
class ActivityPostPage extends React.Component<ActivityPostPageProps, ActivityPostPageState> {
  ...
  async getTxidOfPost(postId: string) {
    let resp = await getDataFromAO(MINI_SOCIAL, 'GetTxid', '0', postId);
    console.log("getTxidOfPost:", resp)
    if (resp.length > 0) return resp[0].txid;
  }
